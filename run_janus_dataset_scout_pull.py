...
from janus_spi.market_dataset_scout import digest, verify_packet
...
def main() -> int:
    ...
    a = p.parse_args()
    repo = Path(a.outbox_repo); answered = Path(a.answered_dir)
    packets_dir = repo / ".janus/market-home-outbox"
    candidates = []
    if packets_dir.is_dir():
        for path in sorted(packets_dir.glob("*.dataset-scout.packet.json")):
            try: packet = json.loads(path.read_text(encoding="utf-8"))
            except Exception: continue
            if not verify_packet(packet): continue
            rid = str(packet["service_request"]["request_id"])
            if (answered / f"{rid}.dataset-scout-result.json").exists(): continue
            candidates.append((str(packet.get("created_at") or ""), rid, path, packet))
    if not candidates:
        write_json(Path(a.status_out), {
            "schema": "janus.home.dataset_scout_pull_status.v1",
            "request_found": False,
            "reason": "NO_UNANSWERED_VALID_DATASET_SCOUT_PACKET",
    ...
    receipt = {
        "schema": "janus.home.dataset_scout_pull_receipt.v1",
        ...
        "dataset_metadata_is_payload": False,
