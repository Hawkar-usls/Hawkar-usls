...
class JanusOIDCPacketRoundtrip:
    def __init__(
        ...
        *,
        ...
        secret_lane: SecretLane | None = None,
    ...
    def run(self, *, source_ref: str, payload: Any, secret_dispatch_token: str, local_github_token: str) -> Dict[str, Any]:
        ...
        activation = JanusActivator(state_dir=self.state_dir, routing_path=self.routing_path, policy_path=self.policy_path).activate(event)
        ...
        # Both lanes are always attempted. A missing secret is recorded, not selected around.
        ...
        if result["oidc_lane_emitted"] is not True or oidc.get("identity_proof") is not True:
            ...
            result["reasons"] = [str(oidc.get("verification_terminal") or oidc.get("terminal"))]
            return result
        ...
        if request_verification.get("ok") is not True:
            ...
            result["reasons"] = [str(request_verification.get("terminal"))]
            return result
        ...
        if observed is None:
            ...
            result["reasons"] = ["No matching target-signed ACK before bounded deadline; silence is not negative evidence."]
            return result
        ...
        result["reasons"] = [
            "Both secret push and OIDC credentialless pull were attempted for one genuine sealed packet.",
            "HOME and Demiurge GitHub Actions identities were independently verified against exact object-bound audiences.",
            "Target execution, P12, launch and every higher authority remain false.",
        ]
        return result
