...
def _oidc_request(url: str, request_token: str, audience: str) -> urllib.request.Request:
    ...
    if parts.scheme != "https" or not host.endswith(".actions.githubusercontent.com"):
        raise ValueError("GITHUB_ACTIONS_OIDC_REQUEST_URL_HOST_REJECTED")
    query = urllib.parse.parse_qsl(parts.query, keep_blank_values=True)
    query.append(("audience", audience))
    bound = urllib.parse.urlunsplit((parts.scheme, parts.netloc, parts.path, urllib.parse.urlencode(query), parts.fragment))
...
def issue_identity_assertion(
    ...
    *,
    ...
    return {
        ...
        "jwt": request_github_oidc_token(audience, opener=opener),
    }
...
def _audience_contains(value: Any, expected: str) -> bool:
    return value == expected if isinstance(value, str) else isinstance(value, list) and expected in value
...
def _authority_false(row: Mapping[str, Any]) -> bool:
    return all(
...
def verify_identity_assertion(
    ...
    *,
    ...
    if assertion.get("schema") != IDENTITY_SCHEMA or assertion.get("provider") != "GITHUB_ACTIONS_OIDC":
        return _failure("OIDC_ASSERTION_SCHEMA_REJECTED", "Identity schema/provider mismatch.")
    if assertion.get("role") != expected_role or assertion.get("audience") != expected_audience:
        return _failure("OIDC_ROLE_OR_AUDIENCE_REJECTED", "Identity role or object audience mismatch.")
    ...
    if missing:
        return _failure("OIDC_REQUIRED_CLAIMS_MISSING", "Missing claims: " + ",".join(missing))
    if claims.get("iss") != ISSUER or not _audience_contains(claims.get("aud"), expected_audience):
        return _failure("OIDC_ISSUER_OR_AUDIENCE_REJECTED", "Issuer or signed audience mismatch.")
    if str(claims.get("repository")) != expected_repository or str(claims.get("repository_id")) != expected_repository_id:
        return _failure("OIDC_REPOSITORY_IDENTITY_REJECTED", "Repository name or immutable ID mismatch.")
    if str(claims.get("repository_owner")) != OWNER or str(claims.get("repository_owner_id")) != OWNER_ID:
        return _failure("OIDC_OWNER_IDENTITY_REJECTED", "Repository owner name or immutable ID mismatch.")
    if str(claims.get("ref")) != MAIN_REF:
        return _failure("OIDC_REF_REJECTED", "Identity did not originate from main.")
    ...
    if iat is None or nbf is None or exp is None or exp <= iat or exp - iat > 900:
        return _failure("OIDC_TIME_CLAIMS_REJECTED", "Token lifetime is invalid.")
    if not (max(iat, nbf) <= bound_at <= exp):
        return _failure("OIDC_BOUND_AT_REJECTED", "bound_at lies outside signed token window.")
    public_claims = {key: claims.get(key) for key in (
        "repository", "repository_id", "repository_owner", "repository_owner_id", "ref",
        "event_name", "workflow_ref", "workflow_sha", "run_id", "run_attempt", "iat", "nbf", "exp",
    )}
    result = {
        "ok": True,
        "identity_proof": True,
...
def verify_request_envelope(envelope: Dict[str, Any], *, decoder: Decoder | None = None) -> Dict[str, Any]:
    ...
    if envelope.get("object_kind") != "DISPATCH_PACKET" or not _authority_false(envelope):
        return _failure("OIDC_REQUEST_AUTHORITY_OR_KIND_REJECTED", "Only authority-bounded packet requests are admitted.")
...
def verify_signed_response(response: Dict[str, Any], *, request_envelope: Dict[str, Any], decoder: Decoder | None = None) -> Dict[str, Any]:
    ...
    if response.get("provenance_class") != PROVENANCE_CLASS or response.get("identity_proof") is not True:
        return _failure("OIDC_RESPONSE_PROVENANCE_REJECTED", "Response provenance class is not admitted.")
    if core.get("request_message_hash") != request_envelope.get("message_hash"):
        return _failure("OIDC_RESPONSE_REQUEST_BINDING_REJECTED", "Response does not bind the exact request.")
    ...
    if not _authority_false(core) or core.get("target_execution_authorized") is not False or core.get("target_execution_performed") is not False:
        return _failure("OIDC_RESPONSE_AUTHORITY_REJECTED", "Response exceeds no-execution authority ceiling.")
    audience = response_audience(str(request_envelope["message_hash"]), core_hash)
    target = verify_target_identity(response.get("target_identity"), audience, decoder=decoder)
    ...
    result = {
        ...
        "target_identity_verification": target,
    }
    result["verification_hash"] = canonical_hash(result)
    return result
...
def verify_no_execution_ack(response: Dict[str, Any], request: Dict[str, Any]) -> bool:
    ...
    payload = core.get("payload") if isinstance(core, dict) else None
...
def _raw_get(url: str, opener: Callable[..., Any]) -> Dict[str, Any] | None:
    try:
        response = opener(urllib.request.Request(url, headers={"User-Agent": "JANUS-HOME-OIDC-Mailbox/1.1"}), timeout=15.0)
        value = json.loads(response.read().decode("utf-8"))
...
class JanusOIDCMailboxTransport:
    def __init__(self, state_dir: str | Path, *, opener: Callable[..., Any] = urllib.request.urlopen, decoder: Decoder | None = None, identity_issuer: IdentityIssuer | None = None) -> None:
    ...
    def publish(self, packet: Dict[str, Any], *, local_github_token: str) -> Dict[str, Any]:
        ...
        existing = _raw_get(self._raw_url(filename), self.opener)
        if existing is not None:
            ...
            if verified.get("ok") is not True or existing.get("object_hash") != packet.get("packet_hash"):
                return {"terminal": "OIDC_MAILBOX_EXISTING_CONFLICT", "published": False, "identity_proof": False}
            path = self._preserve(filename, existing)
            return {
                "terminal": "OIDC_MAILBOX_ALREADY_PUBLISHED_VERIFIED", "published": True, "identity_proof": True,
                "message_hash": existing["message_hash"], "message_path": str(path),
                "object_id": packet["packet_id"], "object_hash": packet["packet_hash"],
                "cross_repository_credential_used": False,
            }
        if not str(local_github_token).strip():
            return {"terminal": "OIDC_MAILBOX_BLOCKED_NO_LOCAL_GITHUB_TOKEN", "published": False, "identity_proof": False}
        audience = request_audience("DISPATCH_PACKET", str(packet["packet_id"]), str(packet["packet_hash"]))
        issuer = self.identity_issuer or (lambda aud: issue_identity_assertion(aud, role="HOME_REQUEST_SOURCE", opener=self.opener))
        envelope = build_request_envelope(packet, issuer(audience))
        verified = verify_request_envelope(envelope, decoder=self.decoder)
        if verified.get("ok") is not True:
            return {
                "terminal": "OIDC_MAILBOX_SELF_IDENTITY_VERIFICATION_FAILED", "published": False, "identity_proof": False,
        ...
        payload = json.dumps({"message": f"Activator OIDC mailbox publish {packet['packet_id']}", "content": encoded, "branch": HOME_BRANCH}, separators=(",", ":")).encode("utf-8")
        request = urllib.request.Request(
            self._api_url(filename), data=payload, method="PUT",
            headers={
                "Authorization": f"Bearer {local_github_token}", "Accept": "application/vnd.github+json",
                "Content-Type": "application/json", "User-Agent": "JANUS-HOME-OIDC-Mailbox/1.1",
        ...
        try:
            ...
            if exc.code in {409, 422}:
                reread = _raw_get(self._raw_url(filename), self.opener)
                if reread == envelope and verify_request_envelope(reread, decoder=self.decoder).get("ok") is True:
                    status = 200
                else:
                    return {"terminal": "OIDC_MAILBOX_PUBLISH_CONFLICT", "published": False, "identity_proof": False, "http_status": exc.code}
            else:
                return {
                    "terminal": "OIDC_MAILBOX_PUBLISH_OUTCOME_UNDETERMINED", "published": False, "identity_proof": True,
                    "message_hash": envelope["message_hash"], "message_path": str(path), "http_status": exc.code,
                }
        return {
            "terminal": "OIDC_MAILBOX_PUBLISHED_AWAITING_SIGNED_ACK" if status in {200, 201} else "OIDC_MAILBOX_PUBLISH_OUTCOME_UNDETERMINED",
            "published": status in {200, 201}, "identity_proof": True,
            "message_hash": envelope["message_hash"], "message_path": str(path),
            "object_id": packet["packet_id"], "object_hash": packet["packet_hash"],
            "cross_repository_credential_used": False, "http_status": status,
        }
...
class JanusOIDCMailboxReader:
    ...
    def response_url(self, request_envelope: Dict[str, Any]) -> str:
    ...
    def read_verified(self, request_envelope: Dict[str, Any]) -> tuple[Dict[str, Any], Dict[str, Any]] | None:
        if verify_request_envelope(request_envelope, decoder=self.decoder).get("ok") is not True:
            raise ValueError("OIDC_LOCAL_REQUEST_NOT_VERIFIED")
        row = _raw_get(self.response_url(request_envelope), self.opener)
        if row is None:
            return None
...
__all__ = [
    ...
    "request_audience", "response_audience", "request_github_oidc_token", "issue_identity_assertion",
    "verify_identity_assertion", "verify_home_identity", "verify_target_identity", "build_request_envelope",
    "verify_request_envelope", "verify_signed_response", "verify_no_execution_ack",
    "JanusOIDCMailboxTransport", "JanusOIDCMailboxReader",
]
