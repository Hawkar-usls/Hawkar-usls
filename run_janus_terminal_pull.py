...
from janus_spi.terminal_mailbox import PublicGitHubMailboxReader, next_unanswered_request


def main() -> int:
    parser = argparse.ArgumentParser(description="Pull the oldest unanswered sealed Terminal conversation message through public GitHub read only")
    ...
    request = next_unanswered_request(reader)
    status = {
